...
interface CrossProduct3DProps {
  width?: number;
  height?: number;
}

export const CrossProduct3D: React.FC<CrossProduct3DProps> = ({
  width = 680,
  height = 460,
}) => {
  ...
  useEffect(() => {
    ...
    // Ejes de referencia en el espacio
    const axesHelper = new THREE.AxesHelper(4);
    scene.add(axesHelper);

    // Cuadrícula en el plano XY (z = 0)
    const grid = new THREE.GridHelper(8, 8, 0x38bdf8, 0x1e2942);
    grid.rotation.x = Math.PI / 2;
    scene.add(grid);

    // Vector A (Cian) sobre el eje X
    ...
    // Vector C = A x B (Verde, perpendicular perpendicular hacia Z)
    ...
    // Plano / Paralelogramo sombreado del área formada por A y B
  ...
  // Actualizar rotación orbital de cámara en cada fotograma
  useEffect(() => {
    if (!cameraRef.current || !rendererRef.current || !sceneRef.current) return;

    const angle = 0.6 + (frame / 60) * 0.25; // Rotación lenta y cinematográfica
    const radius = 9.0;
    const heightY = 4.5;
    ...
    cameraRef.current.up.set(0, 0, 1); // El eje Z apunta hacia arriba
    ...
    <div className="flex flex-col items-center gap-3">
      ...
      <div className="flex items-center gap-6 text-xs text-[#94A3B8]">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-white">w = a × b (Perpendicular Z)</span>
        </div>
      </div>
    </div>
  );
