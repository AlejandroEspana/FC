...
export const DynamicBoardLayout: React.FC<DynamicBoardLayoutProps> = ({
  ...
  return (
    <div
      ...
      className={`relative overflow-hidden select-none font-sans text-white flex flex-col justify-between p-10 ${className}`}
    >
      {/* 1. TEXTURA DE FONDO: Cuadrícula milimétrica técnica */}
      <div
        className="absolute inset-0 pointer-events-none opacity-20"
        style={{
          backgroundImage: `
            linear-gradient(to right, ${theme.boardGrid} 1px, transparent 1px),
            linear-gradient(to bottom, ${theme.boardGrid} 1px, transparent 1px)
          `,
      ...
      <div
        className="absolute -top-32 left-1/2 -translate-x-1/2 w-[1200px] h-[300px] rounded-full blur-[140px] pointer-events-none"
      ...
      <header
        ...
        className="relative z-20 w-full flex items-center justify-between pb-3 border-b border-white/10"
      >
        <div className="flex items-center gap-4">
          ...
          <div
            ...
            className="px-3 py-1 rounded-full border text-[11px] font-mono font-extrabold uppercase tracking-widest shadow-sm flex items-center gap-1.5"
          >
            <span
          ...
          <div>
            ...
            {subtitle && (
              <p className="text-xs text-slate-300 font-normal tracking-wide mt-0.5">
                {subtitle}
              </p>
            )}
          </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1.5 bg-black/30 p-1.5 rounded-full border border-white/10">
      ...
      <footer
        style={{ opacity: headerOpacity }}
        className="relative z-20 w-full flex items-center justify-between pt-2.5 border-t border-white/10 text-xs font-mono text-slate-400"
      >
        {/* Ticker de Insight activo */}
        <div className="flex items-center gap-2.5 min-w-0 flex-1 mr-6">
          <div
            style={{ backgroundColor: `${theme.result}20`, borderColor: `${theme.result}50`, color: theme.result }}
            className="px-2 py-0.5 rounded text-[10px] uppercase font-bold tracking-wider border shrink-0"
          >
            IDEA CLAVE
          </div>
          <span className="text-xs text-slate-200 truncate font-sans font-medium">
