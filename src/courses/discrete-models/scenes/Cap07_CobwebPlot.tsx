...
export const Cap07_CobwebPlot: React.FC = () => {
  ...
  return (
    ...
    >
      <BoardWipeTransition
        durationInFrames={meta.durationFrames || 700}
        ...
        <div className="grid grid-cols-12 gap-6 w-full items-center h-full px-2">
          ...
          <div className="col-span-7 flex flex-col items-center justify-center">
            ...
            >
              <div className="flex items-center justify-between w-full px-2 mb-2 font-mono text-xs">
                <span className="text-emerald-800 font-bold bg-emerald-50 px-2 py-0.5 rounded border border-emerald-200">Curva f(x) = {r}x(1-x)</span>
                <span className="text-amber-800 font-bold bg-amber-50 px-2 py-0.5 rounded border border-amber-200">Semilla Inicial x₀ = {x0}</span>
              </div>
              ...
              <CoordinatePlane
                ...
                stepY={0.2}
              >
                {/* Recta identidad y = x visible sobre fondo blanco */}
                <FunctionGraph
                  f={(x) => x}
                  xRange={[0, 1]}
                  color="#64748B"
          ...
          <div className="col-span-5 flex flex-col gap-3.5">
            ...
            <BoardCallout
              ...
              math={"|f'(x^*)| < 1 \\implies \\text{Orbita Asintoticamente Estable}"}
            />
          </div>
        </div>
      </BoardWipeTransition>
